...
Escape で終了します。矢印キーがX方向パッド（↑→↓←）、WASDがY方向パッド（WDSA）、X/ZがA/B、ReturnがStartです。これは元Swaniumと同じ初期割り当てです。F5/F9はROMごとのステート保存・復元です。ネイティブのコントロールウィンドウにはROM名、FPS、音量スライダーがあり、メニューからROMを開く、停止・再開、ステート保存・復元ができます。`--headless-frames COUNT` を付けると画面を開かずに起動後の実行を確認できます。
