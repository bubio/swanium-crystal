...
これは整形チェック、画面fixtureを含む `crystal spec`、`.app` バンドルのビルド、自作カートリッジを使うCLIの900フレーム実行、SDL2 dummyビデオドライバの起動を実行する。`mise run build` は `bin/Swanium Crystal.app` を生成する。SDL2の最小確認はGUIセッションで `mise run sdl-smoke`、画面・入力・60 Hz提示の確認は `mise run video-demo` を使う。
...
macOS の市販ゲーム互換性は、利用者が合法的に保持する ROM をローカルで `"./bin/Swanium Crystal.app/Contents/MacOS/swanium-crystal" --rom PATH` と明示指定して手動確認する。画面を使わない起動・長時間実行の確認には `--headless-frames COUNT` を併用する。タイトルごとに起動、入力、映像、音声、セーブ、一定時間の連続実行を確認し、結果と既知の制限だけを `docs/compatibility.md` に記録する。市販 ROM、そのハッシュ、セーブデータ、スクリーンショットをリポジトリまたは CI に置かない。
