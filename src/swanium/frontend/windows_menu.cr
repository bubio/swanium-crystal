...
module Swanium::Frontend::WindowsMenu
  ...
  def self.status(title : String, fps : String) : Nil
    LibWindowsMenu.status(title, fps)
  end

  def self.attach_status : Nil
    LibWindowsMenu.attach_status
  end

  def self.status_height : Int32
    LibWindowsMenu.status_height
  end
  ...
  def self.volume : Int32
    LibWindowsMenu.volume
  end

  def self.error(text : String) : Nil
    LibWindowsMenu.error(text)
  end

  def self.about : Nil
    LibWindowsMenu.about
  end

  def self.settings : Nil
    LibWindowsMenu.settings
  end
  ...
  def self.smoke_open_settings : Nil
    LibWindowsMenu.smoke_open_settings
  end

  def self.smoke_settings_tab_count : Int32
    LibWindowsMenu.smoke_settings_tab_count
  end

  def self.smoke_select_settings_tab(index : Int32) : Bool
    LibWindowsMenu.smoke_select_settings_tab(index) != 0
  end

  def self.smoke_settings_tabs_named : Bool
    LibWindowsMenu.smoke_settings_tabs_named != 0
  end

  def self.smoke_status_layout_valid : Bool
    LibWindowsMenu.smoke_status_layout_valid != 0
  end

  def self.smoke_begin_keyboard_capture(index : Int32) : Nil
    LibWindowsMenu.smoke_begin_keyboard_capture(index)
  end

  def self.smoke_send_key(virtual_key : Int32) : Nil
    LibWindowsMenu.smoke_send_key(virtual_key)
  end

  def self.smoke_pending_action : Int32
    LibWindowsMenu.smoke_pending_action
  end

  def self.smoke_keyboard_capture : Int32
    LibWindowsMenu.smoke_keyboard_capture
  end

  def self.smoke_last_taken_action : Int32
    LibWindowsMenu.smoke_last_taken_action
  end

  def self.smoke_focus_volume : Nil
    LibWindowsMenu.smoke_focus_volume
  end

  def self.smoke_game_has_focus : Bool
    LibWindowsMenu.smoke_game_has_focus != 0
  end

  def self.smoke_post_game_key(virtual_key : Int32) : Nil
    LibWindowsMenu.smoke_post_game_key(virtual_key)
  end

  def self.smoke_close_settings : Nil
    LibWindowsMenu.smoke_close_settings
  end

  def self.destroy : Nil
    LibWindowsMenu.destroy
  end
end
