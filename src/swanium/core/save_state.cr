  ...
  module Core
    ...
    module SaveState
      ...
      def self.dump(machine : Machine, bus : WonderSwanBus) : Bytes
        ...
        write_bytes(io, bus.work_ram_snapshot)
        write_bytes(io, bus.save_ram_snapshot)
        write_bytes(io, bus.ports_snapshot)
        bus.save_rtc_state(io)
        bus.save_dma_state(io)
      ...
      private def self.load_unchecked(data : Bytes, machine : Machine, bus : WonderSwanBus) : Nil
        ...
        work_ram = read_bytes(io, bus.work_ram_size)
        save_ram = read_bytes(io, bus.save_ram_size)
